...
def index(request):
    ...
    try:
        from lib.cuckoomx.core.database import Database as Database_CuckooMX
        
        dbcuckoomx = Database_CuckooMX()
    except (CuckooDependencyError, ImportError) as e:
        sys.exit("ERROR: Missing dependency: {0}".format(e))

    report = dict(
        total_mails=dbcuckoomx.count_mails(),
        total_malwares=dbcuckoomx.count_malwares(),
        total_attachments=dbcuckoomx.count_attachments(),
        total_urls=dbcuckoomx.count_urls(),
        mails_have_malwares=dbcuckoomx.get_mails_have_malwares(),
